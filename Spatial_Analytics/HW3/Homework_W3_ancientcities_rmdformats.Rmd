...
- The unit is meter. I can see that from st_crs(cities_sf3035) in the console output: LENGTHUNIT["metre"...
...
#using this mode I can see the context of the map, meaning, where the cities actually are so it is definitely helpful for me to easily interpret the map, however, I can= t see the compass, credits or the scale bar which leads me to believe both modes are very useful when working with buffers 
...
# download.file("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json", "C:/Users/alexp/Documents/School/Spatial analytics/HW/HW3/cds-spatial/Week03/inscriptions.json")

# Load it into R from wherever you put it, and convert into a tibble
list_json <- jsonlite::fromJSON("C:/Users/alexp/Documents/School/Spatial analytics/HW/HW3/cds-spatial/Week03/inscriptions.json")
...
i_sm <- inscriptions %>% 
  ...
  mutate(latitude = as.numeric(gsub(")","", gsub(" ","", latitude))),
         longitude = as.numeric(gsub("c","", gsub("\\(", "", longitude)))) #I did nested gsub()

# Apply the pipeline to the whole dataset, once happy with the result
i <- inscriptions %>% 
  separate(col = coordinates, into = c("longitude","latitude"), sep = ",") %>%
  mutate(latitude = as.numeric(gsub(")","", gsub(" ","", latitude))),
         longitude = as.numeric(gsub("c","", gsub("\\(", "", longitude))))


# Check the result of the subset, does the location look reasonable?
leaflet(options = leafletOptions(preferCanvas = TRUE)) %>% addTiles() %>% addMarkers(lng=i_sm$longitude,lat=i_sm$latitude)
#my RStudio froze twice so I checked the small dataset instead, it really did not like the amount of data
```
...
- most of them were from Italy and few were outside
...
i <- inscriptions %>% 
  ...
  mutate(latitude = as.numeric(gsub(")","", gsub(" ","", latitude))),
         longitude = as.numeric(gsub("c","", gsub("\\(", "", longitude)))) %>%
  filter(!is.na(latitude)) %>%  #filtering NAs
  filter(!is.na(longitude)) %>% 
  filter(longitude > 5 && longitude < 20) #filtering everything smaller than 5 and larger than 20

# Check longitude range
hist(i$longitude) #seems correct

# Create a sf object
i_sf4326 <- st_as_sf(i, coords = c("longitude", "latitude"), crs = 4326)
```

### Question: 
*3. Why are we using EPSG 4326 as the value for CRS in the inscriptions? *

- because it is numeric reference to WGS84 consisting of datum, Latitude-Longitude in degrees which is what our data is in and we want the objects to be in the same coordinate system
...
insc_urban <- st_intersection(i_sf3035, c_buff)
insc_rural <- st_difference(i_sf3035, c_buff)

# calculating ratio
nrow(insc_urban)/nrow(i_sf3035)
...
- around 86.68% is urban inscriptions, in other way, there is around 6.5 times more urban inscriptions than rural ones
