...
```{r}
Py <- mb_geocode("Py, 66360, France ") #very lovely village in Pyrenees mountains in France
Py
...
```{r}
#prep
ke <- mb_geocode("Srobarova 1, Kosice,  04001, Slovakia ") #we get lat, long for the address
ke
...
leaflet() %>%
  addMapboxTiles(style_id = "outdoors-v11",
                 username = "mapbox") %>%
  addPolygons(data =ke_iso_cycle, 
              color = rev(colors),
              fillColor = rev(colors),
              fillOpacity = 0.5, 
              opacity = 0.5, 
              weight = 0.2) %>% 
  addLegend(labels = c(5, 10, 20),
            colors = colors,
            title = "Biking-time<br/>around Srobarova")


##2
route_ke <- mb_directions(origin = ke,
                       destination = "Kaštieľná 6, 049 21 Betliar, Slovakia",
                       profile = "driving")

mapbox_map %>%
  addPolylines(data = route_ke, 
               popup = paste0(
                 "Distance (km): ",
                 round(route$distance, 1), 
                 "<br/>Time (minutes): ",
...
```{r save-iso}
dir.create("C:/Users/alexp/Documents/School/Spatial analytics/HW/HW3/cds-spatial/Week04/data")
library(sf)
write_sf(isos, "C:/Users/alexp/Documents/School/Spatial analytics/HW/HW3/cds-spatial/Week04/isos4326.shp")
```
...
library(raster)
# ?writeRaster()
writeRaster(iso_surface100, "C:/Users/alexp/Documents/School/Spatial analytics/HW/HW3/cds-spatial/Week04/data/iso_surface100.tif", format = "GTiff" )
```
...
OPENCAGE_KEY="701ec75224334110b133c24c14c14b5c"
# set key in the environment
Sys.setenv(OPENCAGE_KEY = "701ec75224334110b133c24c14c14b5c")
...
# Save object
write_csv(h, "C:/Users/alexp/Documents/School/Spatial analytics/HW/HW3/cds-spatial/Week04/data/midty_hospitals.csv")
```
...
```{r}
regions <- getData("GADM", country = "DNK", level = 1)
regions_tf <- st_transform(st_as_sf(regions), crs = 32618)
regions_tf <- st_transform(regions_tf, 25832)
#loading data for Midtylland and transforming to same epsg
st_crs(regions)

plot(st_geometry(regions_tf), col = "grey", border = "white")
#plotting to check the regions
mydt_tf <- regions_tf[2, ]
#extract only Midtjylland
sum(st_area(driving_isos))/st_area(mydt_tf)
#it would seem 65% of Midtjylland is able to drive to hospital within 20 minutes
sum(st_area(cycling_isos))/st_area(mydt_tf)
#almost 5% can bike there

st_area(mydt_tf) - sum(st_area(cycling_isos))
#12 477 849 421 meters squared or 12477.85km^2 is the area unable to reach hospital within 20 minutes
...
```{r}
# Driving range
all_driving_isos <- mb_isochrone(
  h_sf,
  profile = "driving",
  time = 35,
  id = "placename"
)
#I just changed the time around until the area was covered, I changed color to red for easier visibility
mapbox_map %>%
  addPolygons(data = walking_isos,
              popup = ~id) %>% 
   addPolygons(data=all_driving_isos, color = "red")

#it would seem 35 minutes just about covers everyone, otherwise there are few small gaps on the coasts and in the center which would be left out
...
no_cars <- read.csv("C:/Users/alexp/Documents/School/Spatial analytics/HW/HW3/cds-spatial/data/no_cars.csv", header = FALSE)
names(no_cars) <- c("V1", "municipality","nc2020", "nc2021") 
no_cars[1] <- NULL
```
...
households2020_2021 <- read.csv("~/Documents/School/Spatial analytics/HW/HW3/cds-spatial/data/households2020_2021.csv", header=FALSE, sep=";")
#load data

names(households2020_2021) <- c("status", "municipality","f2020", "f2021") 

hh_mun = households2020_2021 %>% group_by(municipality)  %>%
                    summarise(total_2020 = sum(f2020),
                              total_2021 = sum(f2021),
                              .groups = 'drop')

hh_nocar <- merge(hh_mun, no_cars, by.x = "municipality", 
             by.y = "municipality", all.x = TRUE, all.y = FALSE)


hh_nocar <- hh_nocar %>% mutate(Percentage2020 = hh_nocar$nc2020/hh_nocar$total_2020)
hh_nocar <- hh_nocar %>% mutate(Percentage2021 = hh_nocar$nc2021/hh_nocar$total_2021)

pop <- read.csv("~/Documents/School/Spatial analytics/HW/HW3/cds-spatial/data/munic_pop_2022Q1.csv", header=FALSE, sep=",")
pop <- pop[-c(1, 2), ] 
names(pop) <- c("municipality", "pop" )
...
municipalities_tf <- st_transform(st_as_sf(municipalities), crs = 32618)
municipalities_tf <- st_transform(municipalities_tf, 25832)

no_cars_pct_mun <- merge(municipalities_tf, hh_nocar, by.x = "NAME_2", 
             by.y = "municipality", all.x = TRUE, all.y = TRUE)
...
```{r thematic-map}
library(mapview)
mapview(no_cars_pct_mun, zcol = "percentage2020")
